...
export default function AdminPage() {
  ...
  async function loadProducts() {
    ...
    const { data: slideData } = await supabase.from("homepage_slides").select("*").order("sort_order", { ascending: true }).order("created_at", { ascending: false });
    setSlides(slideData || []);
  ...
  async function addSlide(event) {
    ...
    try {
      ...
      const { error: uploadError } = await supabase.storage.from("product-images").upload(path, slideFile,{cacheControl:"3600",upsert:false,contentType:slideFile.type||"image/jpeg"});
      if(uploadError) throw uploadError;
      ...
      setSlideFile(null); setNotice("Hero image added."); await loadProducts();
    } catch(e){setError(e.message||"Could not add hero image.");} finally{setSaving(false);}
  }
  async function toggleSlide(slide){ const {error:e}=await supabase.from("homepage_slides").update({active:!slide.active}).eq("id",slide.id); if(e)setError(e.message); else{setNotice(slide.active?"Hero image hidden.":"Hero image is live.");loadProducts();} }
  async function deleteSlide(slide){ if(!window.confirm("Remove this hero image?"))return; const {error:e}=await supabase.from("homepage_slides").delete().eq("id",slide.id); if(e)setError(e.message); else{setNotice("Hero image removed.");loadProducts();} }
